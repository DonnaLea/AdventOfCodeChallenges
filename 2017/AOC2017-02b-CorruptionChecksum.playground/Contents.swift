//: Advent of Code 2017: 2 - Corruption Checksum (Part 2)
...
func calculateChecksum(input: String) -> Int {
  ...
  for rowString in rowStrings {
    ...
    checksum += divisibleValue(input: rowNumbers)
  }

  return checksum
}

func divisibleValue(input: [Int]) -> Int {
...
class ChecksumTests: XCTestCase {
  func testInput1() {
    let checksum = calculateChecksum(input: """
      ...
      3 8 6 5
      """)
    XCTAssertEqual(checksum, 9)
  }

  func testMinMaxDiffRow1() {
    let diff = divisibleValue(input: [5, 9, 2, 8])
    XCTAssertEqual(diff, 4)
  }

  func testMinMaxDiffRow2() {
    let diff = divisibleValue(input: [9, 4, 7, 3])
    XCTAssertEqual(diff, 3)
  }

  func testMinMaxDiffRow3() {
    let diff = divisibleValue(input: [3, 8, 6, 5])
