...
func bottomTowerName(input: String) -> String {
  ...
  var unownedPrograms = [String]()
  var ownedPrograms = [String]()
  for line in lines {
    let program = Program(input: line)
    programs[program.name] = program
    for child in program.holding {
      if !ownedPrograms.contains(child) {
        ownedPrograms.append(child)
      }
      if let unownedIndex = unownedPrograms.index(of: child) {
        unownedPrograms.remove(at: unownedIndex)
      }
    }
    if !ownedPrograms.contains(program.name) {
      unownedPrograms.append(program.name)
    }
  }

  return unownedPrograms[0]
}
...
struct Program {
  ...
  init(input: String) {
    ...
    weight = UInt(String(input[openBracketIndex..<closeBracketIndex]))!
    if var index = input.index(of: ">") {
      index = input.index(index, offsetBy: 2) // start of the first name
