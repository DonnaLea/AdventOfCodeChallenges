...
func fewestStepsTo(neswAxis: Int, senwAxis: Int, nsAxis: Int) -> Int {
  var fewestSteps = 0
  // Calculate which axis' we're on, can only be a max of 2, could be 1 or just none.
  if neswAxis == 0 && senwAxis == 0 && nsAxis == 0 {
    // No step calculations needed.
  } else if neswAxis != 0 && senwAxis == 0 && nsAxis == 0 {
    fewestSteps = abs(neswAxis)
  } else if neswAxis == 0 && senwAxis != 0 && nsAxis == 0 {
    fewestSteps = abs(senwAxis)
  } else if neswAxis == 0 && senwAxis == 0 && nsAxis != 0 {
    fewestSteps = abs(nsAxis)
  } else {
    if neswAxis > 0 && nsAxis > 0 {
      fewestSteps = neswAxis + nsAxis
    } else if neswAxis > 0 && nsAxis < 0 {
      fewestSteps = max(neswAxis, abs(nsAxis))
    } else if neswAxis > 0 && senwAxis > 0 {
      fewestSteps = neswAxis + senwAxis
    } else if neswAxis > 0 && senwAxis < 0 {
      fewestSteps = max(neswAxis, abs(senwAxis))
    } else if neswAxis < 0 && nsAxis > 0 {
      fewestSteps = max(abs(neswAxis), nsAxis)
    } else if neswAxis < 0 && nsAxis < 0 {
      fewestSteps = abs(neswAxis) + abs(nsAxis)
    } else if neswAxis < 0 && senwAxis > 0 {
      fewestSteps = max(abs(neswAxis), senwAxis)
    } else if neswAxis < 0 && senwAxis < 0 {
      fewestSteps = abs(neswAxis) + abs(senwAxis)
    } else if senwAxis > 0 && nsAxis > 0 {
      fewestSteps = max(senwAxis, nsAxis)
    } else if senwAxis > 0 && nsAxis < 0 {
      fewestSteps = senwAxis + abs(nsAxis)
    } else if senwAxis < 0 && nsAxis > 0 {
      fewestSteps = abs(senwAxis) + nsAxis
    } else if senwAxis < 0 && nsAxis < 0 {
      fewestSteps = max(abs(senwAxis), abs(nsAxis))
    }
  }
...
func fewestStepsTo(input: String) -> Int {
  var neswAxis = 0
  var senwAxis = 0
  var nsAxis = 0
  ...
  for step in path {
    ...
    let currentStepsAway = fewestStepsTo(neswAxis: neswAxis, senwAxis: senwAxis, nsAxis: nsAxis)
    maxStepsAway = max(maxStepsAway, currentStepsAway)
  }

  return fewestStepsTo(neswAxis: neswAxis, senwAxis: senwAxis, nsAxis: nsAxis)
}
...
class Tests : XCTestCase {
  ...
  func testInput1() {
    let input = "ne,ne,ne"
    let steps = fewestStepsTo(input: input)
