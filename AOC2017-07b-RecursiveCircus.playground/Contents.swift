...
func bottomTowerName(input: String) -> String {
  let lines = input.components(separatedBy: CharacterSet.newlines)

  for line in lines {
    let program = Program(input: line)
  ...
  for (_, program) in programs {
    for child in program.holding {
      if var childProgram = programs[child] {
        childProgram.heldBy = program.name
...
struct Program {
  ...
  init(input: String) {
    let spaceIndex = input.index(of: " ")! // move to the space
    name = String(input[..<spaceIndex])
    let openBracketIndex = input.index(after: input.index(of: "(")!) // index of (
    let closeBracketIndex = input.index(of: ")")! // index of )
    weight = Int(String(input[openBracketIndex..<closeBracketIndex]))!
    if var index = input.index(of: ">") {
      index = input.index(index, offsetBy: 2)
      let namesString = input[index...]
      holding = namesString.components(separatedBy: ", ")
    } else {
      holding = []
    }
  }

  func subtowers() -> [Program] {
    var subtowers = [Program]()
    for subtowerName in holding {
      if let subtower = programs[subtowerName] {
  ...
  func isBalanced() -> Bool {
    var isBalanced = true
    var subtowerWeight: Int?

    for subtower in subtowers() {
      if let weight = subtowerWeight {
        if weight != subtower.totalWeight {
          isBalanced = false
          break
        }
      } else {
        subtowerWeight = subtower.totalWeight
      }
    }
  ...
  func unbalancedSubtower() -> Program? {
    var unbalancedSubtower: Program? = nil
    var totalWeights = [Int : Int]()
    var subtowersByWeight = [Int : Program]()

    if isBalanced() == false {
      let subtowers = self.subtowers()
      for subtower in subtowers {
        let count = (totalWeights[subtower.totalWeight] ?? 0) + 1
        totalWeights[subtower.totalWeight] = count
        subtowersByWeight[subtower.totalWeight] = subtower
      }

      // Find the weight with a single count, that is the unbalanced subtower.
      for (key, count) in totalWeights {
        if count == 1 {
          // Look for an inbalance further up the subtower.
          let subtower = subtowersByWeight[key]
          unbalancedSubtower = subtower?.unbalancedSubtower()
          // If no inbalance further up then this is our subtower.
          if unbalancedSubtower == nil {
            unbalancedSubtower = subtower
          }
          break
        }
      }
  ...
  func correctWeight() -> Int? {
    guard let parent = self.parent() else {
      return nil
    }

    var siblingTotalWeight = 0
    for sibling in parent.subtowers() {
      if sibling.name == name {
        continue
      } else {
        siblingTotalWeight = sibling.totalWeight
        break
      }
    }

    let totalWeightDiff = siblingTotalWeight - totalWeight
    let correctWeight = weight + totalWeightDiff
...
cntj (57)
"""
    let name = bottomTowerName(input: input)
    let program = programs[name]!
    XCTAssertEqual(name, "tknk")
    let tower = program.unbalancedSubtower()
    XCTAssertEqual(tower?.name, "ugml")
    XCTAssertEqual(tower?.correctWeight(), 60)
  }
...
"""
    ...
    let program = programs[name]!
    let tower = program.unbalancedSubtower()
    print(tower?.name, tower?.correctWeight())
  }
}
