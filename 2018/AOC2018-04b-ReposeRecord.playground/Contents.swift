...
func calculate(input: String) -> Int {
  ...
  // Save the last sleep record
  if let guardId = currentGuardId, let currentSleepRecord = currentSleepRecord {
    currentGuardSleepRecords.append(currentSleepRecord)
    sleepRecords[guardId] = currentGuardSleepRecords
  }
  
  let minutesMostAsleep = minutesMostAsleepByGuard(sleepRecords: sleepRecords)
  var mostAsleepMinute = -1
  var mostAsleepMinuteGuard = -1
  var mostAsleepMinuteCount = 0
  
  for (guardId, value) in minutesMostAsleep {
    if value.count > mostAsleepMinuteCount {
      mostAsleepMinuteCount = value.count
      mostAsleepMinuteGuard = guardId
      mostAsleepMinute = value.minute
    }
  }
  
  let value = mostAsleepMinuteGuard * mostAsleepMinute
  
  return value
}

func minutesMostAsleepByGuard(sleepRecords: Dictionary<Int, Array<SleepRecord>>) -> Dictionary<Int, (minute: Int, count: Int)> {
  var minutesMostAsleep = Dictionary<Int, (minute: Int, count: Int)>()
  
  for guardId in sleepRecords.keys {
    let value = minuteMostAsleep(guardId: guardId, sleepRecords: sleepRecords)
    minutesMostAsleep[guardId] = value
  }
  
  return minutesMostAsleep
}
...
func minuteMostAsleep(guardId: Int, sleepRecords: Dictionary<Int, Array<SleepRecord>>) -> (minute: Int, count: Int) {
  ...
  return (minute: mostAsleepMinute, count: mostAsleepMinuteCount)
}
