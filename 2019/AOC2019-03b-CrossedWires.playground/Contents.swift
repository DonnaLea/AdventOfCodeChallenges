...
func calculate(input: String) -> Int {
  ...
  var wirePositionsArray: [[Position]] = []
  let lines = input.split(separator: "\n")
  let wireDirectionsArray = lines.map { $0.split(separator: ",") }
  for wireDirections in wireDirectionsArray {
    var positions: [Position] = []
    var lastPositionInDirection: Position = (0, 0, 0)
    for wireInstruction in wireDirections {
      
      let direction = Direction(rawValue: String(wireInstruction.first!))
      let distance = Int(wireInstruction.dropFirst())!
      for i in 1...distance {
        let position: Position
        switch direction {
        case .R:
          position = Position(lastPositionInDirection.x+i, lastPositionInDirection.y, lastPositionInDirection.distance + i)
        case .L:
          position = Position(lastPositionInDirection.x-i, lastPositionInDirection.y, lastPositionInDirection.distance + i)
        case .U:
          position = Position(lastPositionInDirection.x, lastPositionInDirection.y+i, lastPositionInDirection.distance + i)
        case .D:
          position = Position(lastPositionInDirection.x, lastPositionInDirection.y-i, lastPositionInDirection.distance + i)
        default:
          print("unexpected direction: \(String(describing: direction))")
          position = Position(0,0,0)
          break
        }
        positions.append(position)
      }
      lastPositionInDirection = positions.last!
    }
    wirePositionsArray.append(positions)
  }
  
  // analyse where they overlap
  print("analyse overlaps")
  var distances: [Int] = []
//  print("wirePositionsArray: \(wirePositionsArray)")
  for (index, wirePositions) in wirePositionsArray.enumerated() {
    print("index: \(index)")
    if index + 1 > wirePositionsArray.count-1 {
//      print("break")
      break
    }
    let nextWirePositions = wirePositionsArray[index+1]
    print("wirePositions.count: \(wirePositions.count)")
    print("nextWirePositions.count: \(nextWirePositions.count)")
    for (i, position) in wirePositions.enumerated() {
      if i % 100 == 0 {
        print("i: \(i)")
      }
      for nextWirePosition in nextWirePositions {
        if position.x == nextWirePosition.x && position.y == nextWirePosition.y {
          distances.append(position.distance + nextWirePosition.distance)
          print("found matching position: \(position) & \(nextWirePosition)")
          break
        }
      }
    }
  ...
  print("matchingPositions: \(distances)")
  
  // check overlaps for shortest distance
