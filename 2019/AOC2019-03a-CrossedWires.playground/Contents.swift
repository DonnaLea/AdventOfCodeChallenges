...
func calculate(input: String) -> Int {
  ...
  for wireDirections in wireDirectionsArray {
    ...
    var lastPositionInDirection: Position = (0, 0)
    for wireInstruction in wireDirections {
      ...
      for i in 1...distance {
        ...
        switch direction {
        case .R:
          position = Position(lastPositionInDirection.x+i, lastPositionInDirection.y)
        case .L:
          position = Position(lastPositionInDirection.x-i, lastPositionInDirection.y)
        case .U:
          position = Position(lastPositionInDirection.x, lastPositionInDirection.y+i)
        case .D:
          position = Position(lastPositionInDirection.x, lastPositionInDirection.y-i)
        default:
...
//      print("break")
    ...
    for (i, position) in wirePositions.enumerated() {
      ...
      for nextWirePosition in nextWirePositions {
        if position == nextWirePosition {
          matchingPositions.append(position)
          print("found matching position: \(position)")
          break
        }
      }
    }
  ...
  print("matchingPositions: \(matchingPositions)")
