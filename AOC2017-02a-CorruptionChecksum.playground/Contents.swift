...
func calculateChecksum(input: String) -> Int {
  let rowStrings = input.components(separatedBy: CharacterSet(charactersIn: "\n"))
  var checksum = 0
  for rowString in rowStrings {
    print(rowString)
    let rowNumbers = rowString.components(separatedBy: CharacterSet.whitespaces).filter {
      Int($0) != nil
    }.map {
      Int($0)!
    }

    checksum += minMaxDiff(input: rowNumbers)
  }

  return checksum
}

func minMaxDiff(input: [Int]) -> Int {
  var min: Int?
  var max: Int?

  for num in input {
    if let minValue = min, minValue > num {
      min = num
    } else if min == nil {
      min = num
    }

    if let maxValue = max, maxValue < num {
      max = num
    } else if max == nil {
      max = num
    }
...
class ChecksumTests: XCTestCase {
  func testInput1() {
    let checksum = calculateChecksum(input: """
      ...
      2 4 6 8
      """)
    XCTAssertEqual(checksum, 18)
  }

  func testMinMaxDiffRow1() {
    let diff = minMaxDiff(input: [5, 1, 9, 5])
    XCTAssertEqual(diff, 8)
  }

  func testMinMaxDiffRow2() {
    let diff = minMaxDiff(input: [7, 5, 3])
    XCTAssertEqual(diff, 4)
  }

  func testMinMaxDiffRow3() {
    let diff = minMaxDiff(input: [2, 4, 6, 8])
    XCTAssertEqual(diff, 6)
  }
}

ChecksumTests.defaultTestSuite.run()
